...
#define         Side_Selection  Secodary_side  /* 1: primary side; 2: secondary side */

#if          ( Side_Selection == Primary_side )
...
static  void  AppTaskStart (void *p_arg)
{
   ...
   (void)p_arg;
    ...
    AppTaskCreate();        
		...
		//OS_FLAGS flags_cur;
		/* create all the FLAG GRP all system need */
		FLAGS_CTRL_Init();
		
    while (DEF_TRUE) {                                          /* Task body, always written as an infinite loop.       */
        OSTimeDlyHMSM(0u, 0u, 4u, 100u,
                      OS_OPT_TIME_HMSM_STRICT,
                      &err);

			OSFlagPost(&MyEventFlagGrp,
									(OS_FLAGS)(TimeComming), 
